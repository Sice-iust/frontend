...
export default function ThirdPage({ open, onClose, isDarkMode, timeLeft, setTimeLeft, setStep, phoneNumber ,isFinished,setIsFinished}) {
    ...
    const handleNameChange = (event) => {
        ...
        if (numberRegex.test(value)) {  
          if (value.length < 3) {  
              setuserNameError(`طول نام کاربری حداقل ${toPersianDigits("3")} کاراکتر است و  شامل عدد نباید باشد.`);  
          } else {  
            ...
            if (value.length < 3) {  
                setuserNameError(`نام کاربری حتما باید فارسی و حداقل شامل ${toPersianDigits("3")} کاراکتر باشد.`);  
            } else {  
                setuserNameError(`نام کاربری حتما باید فارسی باشد.`);  
            }  
        }   
        else if (value.length < 3) {  
            setuserNameError(`طول نام کاربری حداقل باید ${toPersianDigits("3")} کاراکتر باشد.`);  
        }   
        else {  
            setuserNameError('');  
        }  

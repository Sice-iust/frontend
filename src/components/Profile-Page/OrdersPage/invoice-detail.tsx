...
const invoice: React.FC<invoiceProps> = ({orderId,payment,shippingfee,discount,Product,total_price}) => {
    ...
    return ( 
        <>
        {Product.length > 0 && (
            <div className=" flex flex-col pt-2 lg:p-5">
                {Product.map(({ id, name, price, quantity }) => (
                    <div key={id} className="flex flex-wrap flex-row-reverse py-2">
                       <span className="text-sm font-vazir font-medium text-right ml-auto -ml-auto break-words whitespace-wrap max-w-[45%] sm:w-auto sm:text-base">{name}</span>
                        <div className="flex items-center whitespace-nowrap flex-row-reverse flex-shrink-0 ">
                            ...
                            <span className="text-sm font-vazir font-medium text-right mr-2 text-gray-600 sm:text-base">تومان</span>
                        </div>
                    </div>
                ))}
